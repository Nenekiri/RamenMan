...
public class ClassicRpgGui : MonoBehaviour {
	...
	void OnGUI(){
		...
		float rectX = (!_usingPositionRect) ? Screen.width*0.6f : _positionRect.x; //0.5f
		float rectY = (!_usingPositionRect) ? Screen.height - 100 : _positionRect.y; //100
		float rectWidth = (!_usingPositionRect) ? 300 : _positionRect.width;//512
		float rectHeight = (!_usingPositionRect) ? 100 : _positionRect.height;//190
		...
		dialogueBoxRect.width = Mathf.Clamp(dialogueBoxRect.width, 32, 1000);//2000
		dialogueBoxRect.height = Mathf.Clamp(dialogueBoxRect.height, 32, 1000);//2000
	...
	private void drawDialogueBox(Rect rect){
		drawDialogueBox(rect, new Color(45/255.0F,111f/255.0F, 255/255.0F));//45/255.0F, 111f/255.0F, 255/255.0F
	}
	...
	private void drawDialogueBox(Rect rect, Color color){
		...
		GUI.color = new Color(0,0,0,0);//0.25f
